...
void CalibrateProcess(const PointCloudPtr& input_cloud_ptr, PointCloudPtr out_cloud_ptr, ConfigManager config_manager)
{
    // set extrinct
    ...
    for (size_t i = 0; i < (*input_cloud_ptr).size(); i++)
    {
        ...
        out.SetI(pt.GetI());
        (*out_cloud_ptr).push_back(out);
    }
}

void VisualizationResult(const PointCloudPtr& input_cloud_ptr)
{
    render.RenderPointCloud(input_cloud_ptr, "PointCloud", Color(0,1,0));
}
...
void PcdData(pcl::PointCloud<pcl::PointXYZI>::Ptr& input_cloud_ptr)
{
    ...
    // use to test calibrate params of six dimensions
    PointCloudPtr result_cloud_ptr = std::make_shared<PointCloud>();
    CalibrateProcess(out_cloud_ptr, result_cloud_ptr, config_manager);

    end = clock();
    cout << "The filter calibrate runtime is: " << (float)(end - start) * 1000 / CLOCKS_PER_SEC << "ms" << std::endl;
    cout << "Before the calibrate cloud num is: " << (*out_cloud_ptr).size() << std::endl;
    cout << "After the calibrate cloud num is: " << (*result_cloud_ptr).size() << std::endl;

    // visual
    VisualizationResult(result_cloud_ptr);
    render.KeepWait();
}
...
//    datas_load.LoadPcd(PcdData);
    ...
    **********************/
    ros::init(argc, argv, "calibrate_example_ros");
    ros::NodeHandle node;
