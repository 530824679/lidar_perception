...
void PreProcess(const PointCloudPtr& input_cloud_ptr, const hd_map_msgs::RouteData::ConstPtr p_route_ptr,
                PointCloudPtr out_cloud_ptr, ConfigManager config_manager)
{
    // set extrinct
    ...
    segment->BuildGridMap(*curb_cloud_ptr, *out_cloud_ptr, plane_coefficients);
}

void VisualizationResult(const PointCloudPtr& before_cloud_ptr, const std::vector<PointCloud>& cluster_cloud)
{
    render.RenderDoubleCluster(before_cloud_ptr, cluster_cloud, "PointCloud", Color(0,1,0));
}
...
//     object_cluster->EuclidCluster(*filter_cloud_ptr, cluster_cloud);

//     end = clock();
//     cout << "The cluster object runtime is: " << (float)(end - start) * 1000 / CLOCKS_PER_SEC << "ms" << std::endl;
//     cout << "Before the cluster object cloud num is: " << (*filter_cloud_ptr).size() << std::endl;
//     for (int k = 0; k < cluster_cloud.size(); ++k) {
//         cout << "[cluster object cloud " << k << " :]num is: " << cluster_cloud[k].size() << std::endl;
//     }

//     // Visual
//     VisualizationResult(filter_cloud_ptr, cluster_cloud);
//     render.KeepWait();
// }
...
void RosbagData(const sensor_msgs::PointCloud2ConstPtr& p_Horizon_ptr, const hd_map_msgs::RouteData::ConstPtr p_route_ptr)
{
    ...
    // Use to process the prior steps of passthough
    PointCloudPtr filter_cloud_ptr = std::make_shared<PointCloud>();
    PreProcess(out_cloud_ptr, p_route_ptr, filter_cloud_ptr, config_manager);
    ...
    std::vector<PointCloud> cluster_cloud;
    object_cluster->DBSCANClusterWithMerge(*filter_cloud_ptr, cluster_cloud);
...
int main(int argc, char **argv){
    ...
    **********************/
    ros::init(argc, argv, "cluster_example_ros");
    ros::NodeHandle node;
