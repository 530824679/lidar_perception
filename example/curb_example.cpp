...
void PreProcess(const PointCloudPtr& input_cloud_ptr, PointCloudPtr out_cloud_ptr, ConfigManager config_manager)
{
    ...
    roi_filter->PassThough(calibrate_cloud_ptr, passthough_cloud_ptr);
    voxel_filter->VoxelProcess(*passthough_cloud_ptr, *out_cloud_ptr);
}
...
//     curb_detect->Detect(filter_cloud_ptr, result_cloud_ptr, plane_coefficients);

//     end = clock();
//     cout << "The detect curb runtime is: " << (float)(end - start) * 1000 / CLOCKS_PER_SEC << "ms" << std::endl;
//     cout << "Before the curb detect cloud num is: " << (*filter_cloud_ptr).size() << std::endl;
//     cout << "After the curb detect cloud num is: " << (*result_cloud_ptr).size() << std::endl;
...
void RosbagData(const sensor_msgs::PointCloud2ConstPtr& p_Horizon_ptr)
{
    ...
    // curb_detect->Detect(filter_cloud_ptr, p_GnssImu_ptr, curb_cloud_ptr, result_cloud_ptr, plane_coefficients);
    ...
    //     pt.intensity = (*filter_cloud_ptr)[i].GetI();
    //     (*voxel_pc).push_back(pt);
    // }
    // pcl::toROSMsg(*voxel_pc, voxel_pc_msg);
    // voxel_pc_msg.header.frame_id = "livox_frame";
    // voxel_pc_pub.publish(voxel_pc_msg);

    // pcl::PointCloud<pcl::PointXYZI>::Ptr curb_pc(new pcl::PointCloud<pcl::PointXYZI>);
    // for (size_t i = 0; i < (*curb_cloud_ptr).size(); i++) {
    //     pcl::PointXYZI pt;
    //     pt.x = (*curb_cloud_ptr)[i].GetX();
    //     pt.y = (*curb_cloud_ptr)[i].GetY();
    //     pt.z = (*curb_cloud_ptr)[i].GetZ();
    //     pt.intensity = (*curb_cloud_ptr)[i].GetI();
    //     (*curb_pc).push_back(pt);
    // }
    // pcl::toROSMsg(*curb_pc, curb_pc_msg);
    // curb_pc_msg.header.frame_id = "livox_frame";
    // curb_pc_pub.publish(curb_pc_msg);
    ...
    VisualizationResult(out_cloud_ptr, result_cloud_ptr);
    render.KeepOnce();
}
...
//    datas_load.LoadPcd(PcdData);
    ...
    **********************/
    ros::init(argc, argv, "curb_example_ros");
    ros::NodeHandle node;
