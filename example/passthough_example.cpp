...
//     // Use to test moudle of passthough
//     PointCloudPtr result_cloud_ptr = std::make_shared<PointCloud>();
//     roi_filter->PassThough(filter_cloud_ptr, result_cloud_ptr);

//     end = clock();
//     cout << "The filter passthough runtime is: " << (float)(end - start) * 1000 / CLOCKS_PER_SEC << "ms" << std::endl;
//     cout << "Before the passthough cloud num is: " << (*filter_cloud_ptr).size() << std::endl;
//     cout << "After the passthough cloud num is: " << (*result_cloud_ptr).size() << std::endl;
...
//    datas_load.LoadPcd(PcdData);
    ...
    **********************/
    ros::init(argc, argv, "passthough_example_ros");
    ros::NodeHandle node;
