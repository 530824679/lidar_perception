...
void PreProcess(const PointCloudPtr& input_cloud_ptr, PointCloudPtr out_cloud_ptr, Eigen::Vector4d &plane_coefficients, ConfigManager config_manager)
{
    // set extrinct
    ...
    voxel_filter->VoxelProcess(*passthough_cloud_ptr, *curb_cloud_ptr);
    curb_detect->Detect(curb_cloud_ptr, out_cloud_ptr, plane_coefficients);
}
...
//     PreProcess(out_cloud_ptr, filter_cloud_ptr, plane_coefficients, config_manager);
...
//     segment->BuildGridMap(*filter_cloud_ptr, *result_cloud_ptr, plane_coefficients);

//     end = clock();
//     cout << "The segment ground runtime is: " << (float)(end - start) * 1000 / CLOCKS_PER_SEC << "ms" << std::endl;
//     cout << "Before the segment ground cloud num is: " << (*filter_cloud_ptr).size() << std::endl;
//     cout << "After the segment ground cloud num is: " << (*result_cloud_ptr).size() << std::endl;
...
int main(int argc, char **argv){
    ...
    **********************/
    ros::init(argc, argv, "segment_example_ros");
    ros::NodeHandle node;
