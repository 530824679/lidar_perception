...
#ifndef _LIDAR_PERCEPTION_ROS_MATRIX_CPP_
#define _LIDAR_PERCEPTION_ROS_MATRIX_CPP_

#include "common/utils/matrix.h"
...
/*export*/ template <class T>
    ...
    Matrix<T>::resize(const size_t rows, const size_t columns, const T default_value) {
      ...
      if(rows<0&&columns<0){
        printf("Columns and rows must exist.\n");
        return;
      }
...
/*export*/ template <class T>
    ...
    Matrix<T>::min() const {
      ...
      if(m_rows<0){
        printf("[%s]: Columns and rows must exist.\n");
        return 0;
      }
      //assert ( m_rows > 0 );
      if(m_columns<0){
